...
class PathLength(Feature):
    ...
    def object_path_lengths(log):
        ...
        path_lengths = [len(events) for events in object_events.values()]
        return path_lengths
    ...
    @classmethod
    def path_len_min(cls, log):
        return np.min(cls.object_path_lengths(log))

    @classmethod
    def path_len_max(cls, log):
        return np.max(cls.object_path_lengths(log))

    @classmethod
    def path_len_mean(cls, log):
        return np.mean(cls.object_path_lengths(log))

    @classmethod
    def path_len_median(cls, log):
        return np.median(cls.object_path_lengths(log))

    @classmethod
    def path_len_mode(cls, log):
        return stats.mode(cls.object_path_lengths(log), keepdims=False)[0]

    @classmethod
    def path_len_std(cls, log):
        return np.std(cls.object_path_lengths(log))

    @classmethod
    def path_len_variance(cls, log):
        return np.var(cls.object_path_lengths(log))

    @classmethod
    def path_len_q1(cls, log):
        return np.percentile(cls.object_path_lengths(log), 25)

    @classmethod
    def path_len_q3(cls, log):
        return np.percentile(cls.object_path_lengths(log), 75)

    @classmethod
    def path_len_iqr(cls, log):
        return stats.iqr(cls.object_path_lengths(log))

    @classmethod
    def path_len_skewness(cls, log):
        return stats.skew(cls.object_path_lengths(log))

    @classmethod
    def path_len_kurtosis(cls, log):
        return stats.kurtosis(cls.object_path_lengths(log))
...
# ✅ Expose to feature_extractor.py and CLI
def extract(log):
